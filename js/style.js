   ...
   $('.go-rule').click(function(){
    ...
    var Game = {
        ...
        pageInit:function(){//更换题目
            ...
            $('#change-Q').show();
            $('#next-Q').hide();
            $('#right').removeClass('yes').removeClass('no');
            $('#error').removeClass('no').removeClass('yes');
            if(Game.data.firstInit){
                $('#inner img').attr('src',Game.data.item.inner);
                $('#title span').html(Game.data.item.title);
                $('#right p span').html(Game.data.item.right);
                $('#error p span').html(Game.data.item.error);
                $('.page-03').addClass(Game.data.ranAnimate[Game.rand(5)]);
                Game.data.firstInit = false;
            }else{
                $('.page-03').removeClass('page-03-an01').removeClass('page-03-an02').removeClass('page-03-an03').removeClass('page-03-an04').removeClass('page-03-an05');
        ...
        handle:function(){//操作
            ...
            $('#error').click(function(){
                $('#right').removeClass('yes');
                if(!$('#error').hasClass('yes')){
                    $('#error').addClass('yes');
            ...
            $('#playAgain').click(function(){//再玩一遍
                Game.data = JSON.parse(JSON.stringify(initData));
                Game.pageInit();
                //console.log(Game.data,initData);
                $('.page-03').show();
